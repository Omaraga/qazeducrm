<?php

namespace app\models;

use app\models\relations\EducationGroup;
use app\traits\AttributesToInfoTrait;
use app\traits\UpdateInsteadOfDeleteTrait;
use Yii;
use yii\db\Expression;
use app\components\ActiveRecord;
/**
 * This is the model class for table "pupil_education".
 *
 * @property int $id
 * @property int $pupil_id
 * @property int $tariff_id
 * @property int|null $sale
 * @property string|null $date_start
 * @property string|null $date_end
 * @property string|null $comment
 * @property float|null $tariff_price
 * @property float|null $total_price
 * @property int|null $is_deleted
 * @property string $created_at
 * @property string $updated_at
 * @property int|null $create_user
 * @property int|null $update_user
 * @property string|null $info
 *
 * @property Pupil $pupil
 * @property Tariff $tariff
 * @property EducationGroup[] $groups
 */
class PupilEducation extends ActiveRecord
{
    use UpdateInsteadOfDeleteTrait, AttributesToInfoTrait;

    /**
     * {@inheritdoc}
     */
    public static function tableName()
    {
        return 'pupil_education';
    }

    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['pupil_id', 'tariff_id'], 'required'],
            [['date_start', 'date_end'], 'date', 'format' => 'php:Y-m-d'],
            [['pupil_id', 'tariff_id', 'sale', 'is_deleted', 'create_user', 'update_user'], 'integer'],
            [['date_start', 'date_end', 'created_at', 'updated_at'], 'safe'],
            ['sale', 'number', 'min' => 0, 'max' => 100],
            [['comment', 'info'], 'string'],
            [['tariff_price', 'total_price'], 'number'],
            [['pupil_id'], 'exist', 'skipOnError' => true, 'targetClass' => Pupil::class, 'targetAttribute' => ['pupil_id' => 'id']],
            [['tariff_id'], 'exist', 'skipOnError' => true, 'targetClass' => Tariff::class, 'targetAttribute' => ['tariff_id' => 'id']],
        ];
    }

    /**
     * @return array[]
     */
    public function behaviors()
    {
        return [
            [
                'class' => \yii\behaviors\TimestampBehavior::class,
                'createdAtAttribute' => 'created_at',
                'updatedAtAttribute' => 'updated_at',
                'value' => (new Expression('NOW()')),
            ],
        ];
    }

    public function beforeSave($insert)
    {
        // Null check для консольных команд и API
        $userId = null;
        if (Yii::$app->has('user') && !Yii::$app->user->isGuest && Yii::$app->user->identity) {
            $userId = Yii::$app->user->identity->getId();
        }

        if ($insert && $userId) {
            $this->create_user = $userId;
        }

        if ($userId) {
            $this->update_user = $userId;
        }

        return parent::beforeSave($insert);
    }

    /**
     * {@inheritdoc}
     */
    public function attributeLabels()
    {
        return [
            'id' => Yii::t('main', 'ID'),
            'pupil_id' => Yii::t('main', 'Pupil ID'),
            'tariff_id' => Yii::t('main', 'Тариф'),
            'sale' => Yii::t('main', 'Скидка'),
            'date_start' => Yii::t('main', 'Действует с'),
            'date_end' => Yii::t('main', 'Действует по'),
            'comment' => Yii::t('main', 'Примечание'),
            'tariff_price' => Yii::t('main', 'Tariff Price'),
            'total_price' => Yii::t('main', 'Total Price'),
            'is_deleted' => Yii::t('main', 'Is Deleted'),
            'created_at' => Yii::t('main', 'Created At'),
            'updated_at' => Yii::t('main', 'Updated At'),
            'create_user' => Yii::t('main', 'Create User'),
            'update_user' => Yii::t('main', 'Update User'),
            'info' => Yii::t('main', 'Info'),
        ];
    }

    /**
     * Gets query for [[Pupil]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getPupil()
    {
        return $this->hasOne(Pupil::class, ['id' => 'pupil_id']);
    }

    /**
     * Gets query for [[Tariff]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getTariff()
    {
        return $this->hasOne(Tariff::class, ['id' => 'tariff_id']);
    }

    /**
     * @return \yii\db\ActiveQuery
     */
    public function getGroups(){
        return $this->hasMany(EducationGroup::class, ['education_id' => 'id']);
    }

    /**
     * Проверяет, можно ли редактировать обучение
     * @return bool
     */
    public function canEdit()
    {
        // Проверяем, что обучение принадлежит текущей организации
        if ($this->organization_id && $this->organization_id !== \app\models\Organizations::getCurrentOrganizationId()) {
            return false;
        }

        // Нельзя редактировать удаленные записи
        if ($this->is_deleted) {
            return false;
        }

        return true;
    }

    /**
     * Проверяет, можно ли удалить обучение
     * @return bool
     */
    public function canDelete()
    {
        // Проверяем, что обучение принадлежит текущей организации
        if ($this->organization_id && $this->organization_id !== \app\models\Organizations::getCurrentOrganizationId()) {
            return false;
        }

        // Уже удалено
        if ($this->is_deleted) {
            return false;
        }

        return true;
    }
}
