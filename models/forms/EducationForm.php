...
class EducationForm extends \yii\base\Model
{
    ...
    public function loadDefaultValues()
    {
        ...
        if ($id) {
            ...
            $this->date_start = $education->date_start ? date('d.m.Y', strtotime($education->date_start)) : null;
            $this->date_end = $education->date_end ? date('d.m.Y', strtotime($education->date_end)) : null;
        ...
        } elseif ($this->getScenario() === self::TYPE_COPY) {
            $this->pupil_id = Yii::$app->request->get('pupil_id');
        }
    }
    ...
    public function save()
    {
        ...
        try {
            ...
            $model->date_end = $this->date_end ? date('Y-m-d', strtotime($this->date_end)) : null;
            ...
            foreach ($this->groups as $item) {
                $educationGroup = EducationGroup::find()
                    ->where(['education_id' => $model->id, 'group_id' => $item['group_id']])
                    ->one();
            ...
            // Fix: NOT IN с пустым массивом удаляет ВСЕ записи
            // Удаляем только если есть группы для сохранения
            if (!empty($createdEduGroupIds)) {
                $forDeleteEducationGroups = EducationGroup::find()
                    ->where(['not in', 'id', $createdEduGroupIds])
                    ->andWhere(['education_id' => $model->id])
                    ->all();
            } else {
                // Если нет групп для сохранения, удаляем все связи с этим обучением
                $forDeleteEducationGroups = EducationGroup::find()
                    ->where(['education_id' => $model->id])
                    ->all();
            }
            ...
            $transaction->rollBack();
            Yii::error('Error saving education: ' . $e->getMessage());
            return false;
        }
    }
    /**
    ...
    public function getTariffInfo()
    {
        ...
        if ($tariff->type == 2) {
            ...
            $dateEnd = $this->date_end ? strtotime($this->date_end) : time();
            $dateStart = $this->date_start ? strtotime($this->date_start) : time();
            $days = ($dateEnd - $dateStart) / (24 * 60 * 60);
