...
class Pupil extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['iin'], 'unique', 'targetAttribute' => ['iin', 'organization_id'],
                'message' => 'ИИН уже существует в этой организации'],
    ...
    {
        // Проверка лимита только при создании нового ученика
        if ($insert) {
            $limitService = SubscriptionLimitService::forCurrentOrganization();
            if ($limitService && !$limitService->canAddPupil()) {
                $this->addError('id', SubscriptionLimitService::getLimitErrorMessage('pupil'));
                return false;
            }
        }
    ...
    public function save($runValidation = true, $attributeNames = null)
    {
        ...
        // Безопасная конкатенация ФИО (без null)
        $this->fio = trim(implode(' ', array_filter([
            $this->last_name,
            $this->first_name,
            $this->middle_name
        ])));
